...
export default {
  mounted() {
    ...
    this.$refs.testimonialWrapper?.addEventListener('scroll', this.scrollHandler);
  },
  unmounted() {
    window.removeEventListener('resize', this.resizeHandler);
    this.$refs.testimonialWrapper?.removeEventListener('scroll', this.scrollHandler);
  },
  data() {
    return {
      cardColumns: 3,
  ...
  methods: {
    ...
    scrollNext() {
      this.$refs.testimonialWrapper.scrollLeft += this.$data.newCardWidth + this.$data.cardGap;
    },
    scrollPrevious() {
      this.$refs.testimonialWrapper.scrollLeft -= this.$data.newCardWidth + this.$data.cardGap;
    },
    ...
    updateCardWidth() {
      ...
      (this.$data.cardGap * (this.$data.cardColumns - 1) / this.$data.cardColumns) - 1;
    ...
    updateWrapperClientWidth() {
      this.$data.testimonialsWrapperClientWidth = this.$refs.testimonialWrapper.clientWidth
    },
    updateWrapperScrollLeft() {
      this.$data.testimonialsWrapperScrollLeft = this.$refs.testimonialWrapper.scrollLeft;
      this.$data.testimonialsWrapperMaxScrollLeft = this.$refs.testimonialWrapper.scrollWidth - this.$refs.testimonialWrapper.clientWidth;
    }
  }
}
</script>
...
<template>
  ...
  <div class="slider-nav">
    <button :class="`scroll-button previous ${testimonialsWrapperScrollLeft === 0 ? 'disabled' : ''}`" @click="scrollPrevious" />
    <span class="slider-list-item-rep" v-for="cardData of testimonialCardData" />
    <button :class="`scroll-button next ${testimonialsWrapperScrollLeft === testimonialsWrapperMaxScrollLeft ? 'disabled' : ''}`" @click="scrollNext" />
  </div>
</template>
